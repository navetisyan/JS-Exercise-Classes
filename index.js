...
class Car {
    ...
    drive(distance) {
        let diff = this.tank - distance / this.milesPerGallon;
        if (diff >= 0) {
...
class Instructor extends Lambdasian {
    constructor(opts) {
        ...
        let { specialty, favLanguage, catchPhrase } = opts;
...
class Student extends Lambdasian {
    constructor(opts) {
        ...
        let { previousBackground, className, favSubjects } = opts;
    ...
    sprintChallenge(subject) {
        return `${this.name} has begun a PR for ${subject}`
    }
...
class ProjectManager extends Instructor {
    constructor(opts) {
        super(opts);
        this.gradClassName = opts.gradClassName;
        this.favInstructor = opts.favInstructor;
    }
